...
apt update
apt install -y build-essential python3-dev python3-pip gcc g++ make git build-essential wget python3-pip python3 git-lfs python libglib2.0-dev libfdt-dev libpixman-1-dev zlib1g-dev
...
qemu-system-arm -M mps2-an521 -kernel bl2/ext/mcuboot/mcuboot.axf  -device loader,file=tfm_sign.bin,addr=0x10080000 -serial stdio -display none
